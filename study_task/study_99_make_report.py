...
from study_config import (bids_dir, deriv_dir, report_dir, task, 
...
for sub_string in sub_list:
    ...
    # Load the Epochs
    epoch_fif_file = deriv_path / f'{sub_string}_task-{task}_ref-mastoids_desc-cleaned_epo.fif.gz'
    ...
    for cond, i in evokeds_key.items():
        ...
        figs = []
        evoked = evokeds[i].copy().crop(tmin=-.2, tmax=.6).apply_baseline((-.2,0))
        ...
        report.add_slider_to_section(figs, section='ERP', 
                                 title=f'ERP (Avg. ref.): {cond}', scale=1)
        plt.close()
    
    ## Add Select topos to ERP
    # Scene vs. Objects
    conds = ['scene', 'object']
    these_evokeds = [evokeds[evokeds_key[x]].copy().crop(tmin=-.2, tmax=.6).apply_baseline((-.2,0)) for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Scene and Object Trials',
                                axes='topo', show=False, show_sensors=True)
    fig2 = plot_compare_evokeds(evokeds[evokeds_key['scene-object']].copy().crop(tmin=-.2, tmax=.6).apply_baseline((-.2,0)), 
                                title='Scene and Object Trials - Left Hemisphere',
                                show=False, show_sensors=True,
                                picks=['TP7','P7','PO7'], combine='mean')
    fig3 = plot_compare_evokeds(evokeds[evokeds_key['scene-object']].copy().crop(tmin=-.2, tmax=.6).apply_baseline((-.2,0)), 
    ...
    # Load evokeds
    evoked_fif_file = deriv_path / f'{sub_string}_task-{task}_ref-mastoids_lpf-none_ave.fif.gz'
    evokeds = mne.read_evokeds(evoked_fif_file, verbose=False)
    
    # Load evokeds json
    evoked_json_file = deriv_path / f'{sub_string}_task-{task}_ref-mastoids_lpf-none_ave.json'
    with open(evoked_json_file, 'r') as f:
        evoked_json = json.load(f)
    evokeds_key = evoked_json['evoked_objects']
    ...
    for cond, i in evokeds_key.items():
        ...
        figs.append(evoked.plot_joint(picks=['eeg'], times=times, 
                                      topomap_args=dict(average=.05), title=cond,
                                      show=False))
        figs.append(evoked.plot_topomap(times=np.arange(-.15,evoked.tmax,.1), show=False,
                                         average=.05, nrows=4, title=f'{cond} topomaps (.5s avg)'))
        
        # Add slider section
        report.add_slider_to_section(figs, section='ERP', 
                                 title=f'ERP (Mastoid Ref.): {cond}', scale=1)
        plt.close()
    
    ## Hits vs. Misses (65)
    conds = ['hit65', 'miss65']
    these_evokeds = [evokeds[evokeds_key[x]] for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Hits vs. Misses (65 as old)',
                                axes='topo', show=False, show_sensors=True)
    
    # Plot 100ms time window topomaps of difference wave (min-max scaled) (centered on 0,.5, etc.)
    ...
    conds = ['hit6', 'miss6']
    these_evokeds = [evokeds[evokeds_key[x]] for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Hits vs. Misses (6 as old)',
                                axes='topo', show=False, show_sensors=True)
    ...
    these_evokeds = [evokeds[evokeds_key[x]] for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Scenes: Hits vs. Misses (65 as old)',
                                axes='topo', show=False, show_sensors=True)
    ...
    these_evokeds = [evokeds[evokeds_key[x]] for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Scenes: Hits vs. Misses (6 as old)',
                                axes='topo', show=False, show_sensors=True)
    ...
    these_evokeds = [evokeds[evokeds_key[x]] for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Objects: Hits vs. Misses (65 as old)',
                                axes='topo', show=False, show_sensors=True)
    ...
    these_evokeds = [evokeds[evokeds_key[x]] for x in evokeds_key.keys() if x in conds]
    fig1 = plot_compare_evokeds(these_evokeds, title='Objects: Hits vs. Misses (6 as old)',
                                axes='topo', show=False, show_sensors=True)
