...
from functions import (loadmat, get_sub_list)
...
fit_roc = ("roc_data=roc_solver(targf,luref,model," +
           "fitStat,x0,lb,ub,'subID',sub_id,'condLabels'," +
           "conditions,'figure',false,'verbose',false);")
...
for sub in sub_list:
    ...
    beh_file = bids_dir / sub / \
        'beh' / f'{sub}_task-test_beh.tsv'
    beh_data = pd.read_csv(beh_file, sep='\t')
    memory_bin = []
    study_acc = []
    for row in beh_data.itertuples():
        # Code memory bin
        if (row.item_type == 'old' and row.study_n_responses == 1
           and row.test_n_responses == 1):
    ...
    query = ('((item_type=="old" and study_n_responses==1) or ' +
             '(item_type=="new")) and test_n_responses==1')
    ...
    for r in [6, 5, 4, 3, 2, 1]:
        ...
        for c in ['scene', 'object']:
            for t in ['old', 'new']:
                query = (f"test_resp=={float(r)} and category=='{c}' " +
                         f"and item_type=='{t}'")
                # Get the number of rows as a count
                counts[r][(c, t)] = beh_data.query(query).shape[0]
    ...
    roc_file = str(out_path / f'{sub}_task-test_roc_data.mat')
    ...
    ax2.legend(['scene', 'object'], fontsize=14, loc='upper center',
               frameon=False, mode='expand', ncol=2,
               bbox_to_anchor=(0, 1.05, 1, .12))
    for cond, color in enumerate(colors):
    ...
    mem_bin_count = beh_data.query(query).groupby(
        'category')['memory_bin'].value_counts().unstack('category')
    ...
    ax6 = plt.subplot(444)
    recog_data = pd.DataFrame(np.append(pr, overall_pr), columns=['pR'],
                              index=['scene', 'object', 'overall'])
    ...
    study_median_rts = beh_data.query(query).groupby(
        group_by)['study_rt'].median().to_frame()
    study_mean_rts = beh_data.query(query).groupby(
        group_by)['study_rt'].mean().to_frame()
    study_sd_rts = beh_data.query(query).groupby(
        group_by)['study_rt'].std().to_frame()

    # Plot median RT
    ax7 = plt.subplot(8, 8, (29, 38))
    study_rt_plot = study_median_rts.unstack('category')
    study_rt_plot = study_rt_plot[[('study_rt', 'scene'),
                                   ('study_rt', 'object')]]
    ...
    ax7.set_ylim((np.nanmin(
        study_median_rts.values)*.95, np.nanmax(study_median_rts.values)*1.05))
    ax7.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))

    # Compute study acc measures
    study_acc = beh_data.query(query).groupby(
        group_by)['study_acc'].mean().to_frame()

    # Plot study ACC measures
    ax8 = plt.subplot(8, 8, (31, 40))
    study_acc_plot = study_acc.unstack('category')
    study_acc_plot = study_acc_plot[[('study_acc', 'scene'),
                                     ('study_acc', 'object')]]
    ...
    # Test RTs
    test_median_rts = beh_data.query(query).groupby(
        group_by)['test_rt'].median().to_frame()
    test_mean_rts = beh_data.query(query).groupby(
        group_by)['test_rt'].mean().to_frame()
    test_sd_rts = beh_data.query(query).groupby(
        group_by)['test_rt'].std().to_frame()

    # Plot test RTs
    ax9 = plt.subplot(7, 2, (12, 14))
    test_rt_plot = test_median_rts.unstack('category')
    test_rt_plot = test_rt_plot[[('test_rt', 'scene'),
                                 ('test_rt', 'object')]]
    ...
    ax9.set_ylim(
        np.nanmin(test_rt_plot.values)*.6, np.nanmax(test_rt_plot.values)*1.10)
    ...
    for cond in ['scene', 'object']:
        for itype in ['old', 'new']:
            for col in targf.columns:
                out_dict[f'{cond}_{itype}_conf_{col}'] = \
                    counts.loc[(cond,itype), col]
    ...
    for val in ['median', 'mean', 'sd']:
        ...
        for cond in ['scene', 'object']:
            for mem_bin in ['hit', 'miss']:
                out_dict[f'{cond}_{mem_bin}_{val}_study_rt'] = \
                    this_rt.loc[(cond, mem_bin)]['study_rt']
    ...
    for val in ['median', 'mean', 'sd']:
        ...
        for cond in ['scene', 'object']:
            for mem_bin in ['hit', 'miss', 'cr', 'fa']:
                try:
                    out_dict[f'{cond}_{mem_bin}_{val}_test_rt'] = \
                        this_rt.loc[(cond, mem_bin)]['test_rt']
                except ValueError:
    ...
    pd.DataFrame().from_dict(
        out_dict).to_csv(summary_file, index=False, sep='\t')
