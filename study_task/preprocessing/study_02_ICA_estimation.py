"""
Script: study_02_ICA_estimation.py
Creator: Joshua D. Koen
Description: This script handles ICA estimation for the Monster task.
"""
...
for sub in sub_list:
    ...
    # Resample events and raw to 250Hz (yes this causes jitter,
    ...
    epochs.plot_psd(picks=['eeg'], xscale='linear', show=False, ax=ax2,
                    n_jobs=4)
    plt.show()
    ...
    ext_val_nchan = (np.sum(max_voltage > preprocess_options['ext_val_thresh'],
                     axis=1))
    ext_val_bad = ext_val_nchan >= n_thresh_channels
    print('Epochs with extreme voltage on more than ' +
          f'{n_thresh_channels} channels:', ext_val_bad.nonzero()[0])
    ...
    blink_inds = np.where(
        veog_diff.squeeze() > preprocess_options['blink_thresh'])[0]
    ...
    json_info = {
        ...
        'eog_scores': {
            ...
            'veog_scores':
                {f'comp{i:02d}':
                 float(x) for i, x in enumerate(eog_scores[0])},
            'heog_scores':
                {f'comp{i:02d}':
                 float(x) for i, x in enumerate(eog_scores[1])},
                      }
    }
    json_file = deriv_path / \
