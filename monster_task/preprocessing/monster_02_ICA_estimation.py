"""
Script: monster_02_ICA_estimation.py
...
for sub in sub_list:
    ...
    epochs.plot_psd(
        picks=['eeg'], xscale='linear', show=False, ax=ax2, n_jobs=4)
    plt.show()
    ...
    json_info = {
        ...
        'eog_scores': {
            ...
            'veog_scores': {f'comp{i:02d}':
                            float(x) for i, x in enumerate(eog_scores[0])},
            'heog_scores': {f'comp{i:02d}':
    ...
    json_file = deriv_path / f'{sub}_task-{task}_ref-FCz_desc-resamp_raw.json'
    with open(json_file, 'w') as outfile:
        json.dump(json_info, outfile, indent=4)
    del json_info, json_file
