...
for sub in sub_list:
    ...
    raw_fif_file = deriv_path / \
        f'{sub}_task-{task}_ref-FCz_desc-import_raw.fif.gz'
    ...
    # Resample events and raw to 250Hz
    ...
    event_file = deriv_path / f'{sub}_task-{task}_desc-resamp_eve.txt'
    ...
    json_file = deriv_path / f'{sub}_task-{task}_desc-resamp_eve.json'
    ...
    epochs = mne.Epochs(raw, events, event_id=event_id,
                        ...
                        baseline=(None, None), reject=None,
                        preload=True)
    ...
    json_info = {
        ...
        'eeg_file':  (deriv_path /
                      f'{sub}_task-{task}_ref-FCz_desc-ica_epo.fif.gz').name
    }
    json_file = deriv_path / f'{sub}_task-{task}_ref_FCz_ransac.json'
    with open(json_file, 'w') as outfile:
        json.dump(json_info, outfile, indent=4)
    del json_info, json_file
    ...
    n_good_channels = len(mne.pick_types(epochs.info,
                                         eeg=True, eog=False, exclude='bads'))
    ...
    ext_val_nchan = (np.sum(max_voltage >
                     preprocess_options['ext_val_thresh'], axis=1))
    ext_val_bad = ext_val_nchan >= n_thresh_channels
    print(f'Epochs with extreme voltage on more than {n_thresh_channels}',
          'channels:', ext_val_bad.nonzero()[0])
    ...
    print(f'Epochs exceeding global P2P on more than {n_thresh_channels}',
          'channels:', p2p_bad.nonzero()[0])

    # Detect eog at stim onsets
    veog_data = epochs.copy().apply_baseline((None, None))
    veog_data.crop(tmin=-.1, tmax=.1).pick_channels(['VEOG']).get_data()
    veog_diff = np.abs(veog_data.max(axis=2) - veog_data.min(axis=2))
    blink_inds = \
        np.where(veog_diff.squeeze() > preprocess_options['blink_thresh'])[0]
    ...
    json_file = deriv_path / \
        f'{sub}_task-{task}_ref-FCz_desc-ica_epo.json'
    with open(json_file, 'w') as outfile:
        json.dump(json_info, outfile, indent=4)
    del json_info, json_file
    ...
    # Save ICA
    ica_file = deriv_path / \
        f'{sub}_task-{task}_ref-FCz_desc-ica_ica.fif.gz'
    ica.save(ica_file)
    ...
    json_file = deriv_path / \
        f'{sub}_task-{task}_ref-FCz_desc-ica_ica.json'
    with open(json_file, 'w') as outfile:
        json.dump(json_info, outfile, indent=4)
    del json_info, json_file
    ...
    json_file = deriv_path / \
        f'{sub}_task-{task}_ref-FCz_desc-resamp_raw.json'
    with open(json_file, 'w') as outfile:
        json.dump(json_info, outfile, indent=4)
    del json_info, json_file
